...
impl Aggregator {
    ...
    fn aggregate_by_timerange(
        ...
        AggregatedData {
            ...
            input_tokens,
            output_tokens,
            session_count: filtered_sessions.len() as i64,
            cost_by_model,
            cost_by_provider,
...
mod tests {
    ...
    fn create_test_session(hours_ago: i64, cost: f64) -> Session {
    ...
    fn test_aggregate_daily_filters_correctly() {
        let sessions = vec![
            create_test_session(2, 5.0),  // Within 5 hours
            create_test_session(6, 3.0),  // Outside 5 hours
        ];
    ...
    fn test_aggregate_accumulates_costs_by_model() {
        let mut s1 = create_test_session(2, 5.0);
        s1.model = "claude-opus".to_string();

        let mut s2 = create_test_session(1, 3.0);
    ...
    fn test_aggregate_accumulates_tokens() {
        let sessions = vec![
            create_test_session(2, 5.0),
            create_test_session(1, 3.0),
        ];
