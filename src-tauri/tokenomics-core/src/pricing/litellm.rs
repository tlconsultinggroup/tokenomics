...
impl ModelPricing {
    ...
    /// tokenomics reports both at $0.00 and cannot do better from this input
...
/// `~/.config/tokenomics/cache/pricing-litellm.json`. Any fixture-server test
...
/// `TOKENOMICS_CONFIG_DIR`: the override is process-global, so it only protects
...
async fn fetch_inner(url: &str, use_disk_cache: bool) -> Result<PricingDataset, String> {
    ...
    if use_disk_cache {
        if let Err(e) = cache::save_cache(CACHE_FILENAME, &data) {
            eprintln!(
                "[tokenomics] Warning: Failed to cache LiteLLM pricing at {}: {}",
...
mod tests {
    ...
    // test without any tokenomics defect.
    #[tokio::test]
    ...
    /// `~/.config/tokenomics/cache/pricing-litellm.json`, evicting the genuine
    ...
    ///
    /// The assertion redirects `TOKENOMICS_CONFIG_DIR` at a `TempDir` instead of
    ...
    ///
    /// `#[serial]` is load-bearing for the same reason. `TOKENOMICS_CONFIG_DIR`
