...
/// flag rather than on tests remembering to redirect `TOKENOMICS_CONFIG_DIR`.
async fn fetch_inner(url: &str, use_disk_cache: bool) -> Result<PricingDataset, String> {
    ...
    if use_disk_cache {
        if let Err(e) = cache::save_cache(CACHE_FILENAME, &data) {
            eprintln!(
                "[tokenomics] Warning: Failed to cache models.dev pricing at {}: {}",
...
mod tests {
    ...
    /// why the assertion redirects `TOKENOMICS_CONFIG_DIR` rather than probing
