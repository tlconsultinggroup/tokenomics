...
pub fn parse_opencode_file(path: &Path) -> Option<UnifiedMessage> {
    ...
    // tokenomics's LiteLLM repricing pass. A zero cost usually means OpenCode
...
fn legacy_migration_cache_paths() -> Vec<std::path::PathBuf> {
    ...
    [
        ...
        crate::paths::legacy_dot_cache_tokenomics_dir().map(|d| d.join(MIGRATION_CACHE_FILENAME)),
    ]
    .into_iter()
    .flatten()
    .collect()
...
mod tests {
    ...
    /// the real per-message data. Mirrors the columns tokenomics actually reads.
    ...
    fn test_migration_cache_missing_returns_none() {
        // load_opencode_migration_cache reads from ~/.cache/tokenomics/opencode-migration.json
    ...
    fn migration_record_falls_back_to_legacy_path() {
        ...
        let mut guard = EnvGuard::capture(&["TOKENOMICS_CONFIG_DIR", "XDG_CACHE_HOME", "HOME"]);
        guard.set("HOME", temp_home.path());
        guard.set("XDG_CACHE_HOME", temp_xdg_cache.path());
        guard.remove("TOKENOMICS_CONFIG_DIR");
