...
//! Used by `tokenomics report` to provide task-attributed usage reports.
...
impl WikiDb {
    ...
    /// `dirs::config_dir()` chain this replaces ignored `TOKENOMICS_CONFIG_DIR`
    /// outright: `tokenomics report` opened — and created — `wiki.db` in the
    /// real profile even when every other file was redirected to an isolated
    /// root. On Windows that is `%APPDATA%\tokenomics`, and on macOS
    /// `~/Library/Application Support/tokenomics`, which also contradicted this
    /// very doc comment's `~/.config/tokenomics` claim.
...
mod tests {
    ...
    fn guard() -> EnvGuard {
        EnvGuard::capture(&["TOKENOMICS_CONFIG_DIR"])
    }

    /// `tokenomics report` is the one command that opens a SQLite database, and
    /// it was the one path that ignored `TOKENOMICS_CONFIG_DIR`: the old
    ...
    fn default_path_follows_the_config_dir_override() {
        let mut env = guard();
        let root = std::env::temp_dir().join("tokenomics-wiki-default-path-probe");
        env.set("TOKENOMICS_CONFIG_DIR", &root);
        assert_eq!(WikiDb::default_path(), root.join("wiki.db"));
    }

    /// The override is the hermetic contract, so the wiki must sit under the
    /// same root as every other tokenomics file rather than beside it. Pinned
    ...
    fn default_path_stays_inside_the_resolved_config_dir() {
        let mut env = guard();
        let root = std::env::temp_dir().join("tokenomics-wiki-containment-probe");
        env.set("TOKENOMICS_CONFIG_DIR", &root);
