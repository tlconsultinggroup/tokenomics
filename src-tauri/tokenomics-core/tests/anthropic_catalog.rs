...
//!
//! `#[ignore]`d so CI (which has no `~/.config/tokenomics/cache/`) skips them.
//! Run manually when datasets are cached:
//!
//! ```sh
//! cargo test -p tokenomics-core --test anthropic_catalog -- --ignored
//! ```
//!
...
use tokenomics_core::pricing::lookup::PricingLookup;
use tokenomics_core::pricing::{litellm, models_dev, openrouter, ModelPricing};
